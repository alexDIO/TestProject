...
import transport.storages.TransportStorageHibernate;
...
public class TransportMapHolder {
    ...
    public void deleteTransportByMark(String mark) {
        if (storage instanceof TransportStorageHibernate){
            TransportStorageHibernate hibernateStorage = (TransportStorageHibernate)storage;
            hibernateStorage.deleteTransportByMark(mark);
            try {
                storedTransport = storage.getAllTransport();
            } catch (IOException e) {
                e.printStackTrace();
